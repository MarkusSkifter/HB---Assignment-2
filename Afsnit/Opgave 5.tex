...
When we change $\lambda$, households respond by adjusting their hours. So for each candidate value of $\lambda$, we need to fully re-solve the model and re-simulate it to get the new tax revenue. The steps are:
...
We first evaluate $\Delta G$ on a coarse grid of $\lambda$ values between 1.5 and 3.0 to find a bracket where the function changes sign. Then we use Brent's method (\texttt{scipy.optimize.brentq}) to pin down the exact root within that bracket. Since a higher $\lambda$ uniformly lowers the tax paid on any income, $\Delta G(\lambda)$ is monotonically decreasing over this range, so the budget-neutral root is unique.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Billeder/Budget difference.png}
    \caption{Budget difference $\Delta G(\lambda^{\text{indiv}})$ as a function of the individual tax scale; the root at $\lambda^{\text{indiv},\star} \approx 1.767$ marks budget neutrality.}
    \label{fig:budget_diff}
\end{figure}
