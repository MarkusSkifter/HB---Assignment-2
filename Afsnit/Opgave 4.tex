...
\begin{tabular}{l r}
\hline
Lifetime revenue, joint taxation      & 3{,}610.5 \\
Lifetime revenue, individual taxation & 5{,}343.0 \\
\hline
Budget change ($\Delta G$)            & $+$1{,}732.5 \\
Change in percent                     & $+$47.99\% \\
\hline
\end{tabular}
\end{center}

So switching to individual taxation \emph{increases} government revenue by about 48\%. This happens because the individual tax system encourages member 2 to work more (as discussed in Question~3), which raises total household income and therefore total tax payments, even though the marginal tax rate on the secondary earner is lower.

This is a Laffer-type response: the lower marginal rate on the secondary earner is more than offset by the broader tax base as labor supply rises. It also shows that the Question~3 comparison is not revenue-neutral --- individual taxation lowers rates \emph{and} raises revenue --- which motivates the budget-neutral experiment in Question~5.
...
\begin{figure}
    ...
    \caption{Average per-period tax revenue per household under joint and individual taxation; individual taxation generates higher revenue in every period.}
    \label{fig:placeholder}
\end{figure}

\subsection*{Note on Discounting}

We use the undiscounted sum as the primary measure because it represents the government's actual cumulative tax intake over the life cycle --- the total amount of resources collected regardless of when they arrive. As a robustness check, discounting each period's revenue by $\beta^t = 0.98^t$ gives a present value of the budget change of 1{,}592.8, which is still a large positive number. This confirms that the revenue gain is not driven by a few late periods but is present throughout the life cycle, and the conclusion is robust to the choice of discounting.
