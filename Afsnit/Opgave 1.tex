...
\end{equation}
%
so member~1 carries the labour-disutility weight $\mu$ and member~2 the weight $(1-\mu)$.

\subsection*{Implementation}

We add the parameter \texttt{par.mu = 0.5} to the model class. In \texttt{util()} the labour disutility of member~1 is weighted by \texttt{par.mu} and that of member~2 by \texttt{1-par.mu}, with consumption entering as \texttt{(cons/2)**(1+eta)/(1+eta)}.

At $\mu = 0.5$ the weighted utility is a positive rescaling of the baseline (see Question~2), and because utility is ordinal the optimal labour supply is unaffected. Solving and simulating both specifications confirms this: the average hours coincide down to the optimizer's numerical tolerance,
%
\begin{align*}
